...
export declare type onMessageFunc<T extends ISocketJSONType> = (data: unknown | never, json: T) => void;
...
export interface SocketResponse<T extends ISocketJSONType> extends SocketState<T> {
    connect: (params: IConnect<T>) => WebSocket;
    socket: WebSocket;
    sendData: (data: T) => void;
}
export interface IConnect<T extends ISocketJSONType> extends SocketEvents<T>, SocketProps {
    endpoint?: string;
}
